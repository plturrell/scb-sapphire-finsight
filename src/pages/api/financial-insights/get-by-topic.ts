...
/**
 * Get financial insights by topic API endpoint
 * 
 * Retrieves financial insights for a specific topic
 * 
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const parsedLimit = limit ? 
    (Array.isArray(limit) ? parseInt(limit[0], 10) : parseInt(limit, 10)) : 5;
  ...
  try {
    ...
    const insights = await financialInsightsService.getInsightsByTopic(topic, {
      limit: parsedLimit,
      offset: parsedOffset,
    });
