...
import { Grok3AnalysisRequest } from '../types/GrokTypes';
...
import { VietnamMonteCarloConfig, TariffParameter } from '../components/VietnamMonteCarloParams';
  ...
  public convertToStorageModel(
    ...
    const exchangeRates = config.financialParameters
      ...
      .map(p => ({
        ...
        minRate: p.min !== undefined ? (typeof p.min === 'number' ? p.min : parseFloat(p.min.toString())) : 0,
        maxRate: p.max !== undefined ? (typeof p.max === 'number' ? p.max : parseFloat(p.max.toString())) : 0
      }));
    ...
    const generalParameters: SimulationParameter[] = [
      ...config.tariffParameters
        ...
        .map(p => ({
          ...
          name: p.name,
          value: p.value,
          distributionType: p.distribution,
          parameterType: p.unit === '%' ? 'Percentage' : 'Numeric',
          min: p.min,
          max: p.max,
          unit: p.unit,
      ...
      ...config.financialParameters
        ...
        .map(p => ({
          ...
          value: p.value,
          distributionType: p.distribution,
          parameterType: p.unit === 'USD' || p.unit === 'VND' ? 'Currency' : 'Numeric',
          min: p.min,
          max: p.max,
          unit: p.unit,
          description: p.description
  ...
  public convertToComponentConfig(input: SimulationInput): VietnamMonteCarloConfig {
    ...
    const tariffParameters = [
      ...
      ...input.parameters.generalParameters
        ...
        .map(p => ({
          ...
          name: p.name,
          value: p.value,
          min: p.min || p.minValue,
          max: p.max || p.maxValue,
          distribution: p.distribution || p.distributionType || 'Normal',
          description: p.description,
          unit: p.unit
        }) as ComponentParameter) // Create a copy to avoid mutating the original
    ];
    ...
    const financialParameters = [
      ...
      ...input.parameters.generalParameters
        ...
        .map(param => {
          // Create direct object to avoid conversion issues
          const converted = {
            id: param.id,
            name: param.name,
            value: param.value,
            min: param.min || param.minValue,
            max: param.max || param.maxValue,
            distribution: param.distribution || param.distributionType || 'Normal',
    ...
    const config: VietnamMonteCarloConfig = {
      ...
      tariffParameters: tariffParameters as TariffParameter[],
      financialParameters: financialParameters as TariffParameter[],
      simulationSettings
    };
