...
import { OntologyManager, ExtendedOntologyManager } from './OntologyManager';
...
export interface ResilienceAnalysisOptions {
  ...
  enhanceWithPerplexity: boolean; // Use advanced Perplexity NLP for enhanced analysis
  perplexityAnalysisDepth?: 'basic' | 'standard' | 'comprehensive'; // Depth of perplexity analysis
  depth?: number; // Depth of supply chain tiers to analyze
  includeTariffImpact?: boolean; // Whether to include tariff impact in analysis
  includeGeopolitical?: boolean; // Whether to include geopolitical risk in analysis
}
...
class SupplyChainResilienceAnalyzer {
  ...
  constructor(ontologyManager: OntologyManager) {
    ...
    try {
      ...
      console.log('SupplyChainResilienceAnalyzer initialized with Perplexity NLP integration');
    } catch (error) {
      console.error('Failed to initialize Perplexity NLP integration:', error);
    }
  ...
  async analyzeResilience(entityId: string, options: Partial<ResilienceAnalysisOptions> = {}): Promise<SupplyChainResilience> {
    ...
    // Decide if we should use Perplexity enhanced analysis
    const usePerplexity = options.enhanceWithPerplexity !== false && this.perplexityNLP !== null;
    const perplexityDepth = options.perplexityAnalysisDepth || 'standard';
    
    // Generate a unique cache key including Perplexity options
    const cacheKey = `${entityId}-${JSON.stringify({...options, enhanceWithPerplexity: usePerplexity, perplexityDepth})}`;
    const cachedResult = this.resilienceCache.get(cacheKey);
    
    if (cachedResult && (Date.now() - cachedResult.timestamp) < this.CACHE_TTL) {
      console.log('Returning cached resilience analysis');
      
      // Send notification about using cached data
      if (cachedResult.data.lastUpdated) {
        const lastUpdatedTime = new Date(cachedResult.data.lastUpdated).toLocaleString();
        NotificationCenter.showNotification({
          title: 'Using cached supply chain data',
          body: `Using saved supply chain analysis from ${lastUpdatedTime}`,
          priority: 'low',
          category: 'data',
          autoClose: 5000
        }, 'supply-chain', 'data-cache');
      }
    ...
    const analysisOptions: ResilienceAnalysisOptions = {
      ...
      includeTariffAnalysis: options.includeTariffAnalysis !== false,
      includeComplianceData: options.includeComplianceData !== false,
      includeAlternativeSuppliers: options.includeAlternativeSuppliers !== false,
      monteCarloSimulations: options.monteCarloSimulations || this.DEFAULT_MONTE_CARLO_ITERATIONS,
      confidenceLevel: options.confidenceLevel || this.DEFAULT_CONFIDENCE_LEVEL,
      timeHorizon: options.timeHorizon || 12,
      enhanceWithPerplexity: usePerplexity,
      perplexityAnalysisDepth: perplexityDepth,
    ...
    // Enhance analysis with Perplexity NLP if enabled
    let perplexityEnhancedData = {};
    if (usePerplexity && this.perplexityNLP) {
      try {
        console.log(`Enhancing supply chain analysis with Perplexity NLP (depth: ${perplexityDepth})`);
        
        // Create a detailed analysis prompt based on preliminary results
        const analysisPrompt = this.createPerplexityAnalysisPrompt(
          entityId, 
          vulnerabilityHotspots, 
          monteCarloResults, 
          recommendedActions,
        ...
        const perplexityResult = await this.perplexityNLP.processQuery({
        ...
        perplexityEnhancedData = {
          enhancedInsights: perplexityResult.insights,
          enhancedRecommendations: this.extractEnhancedRecommendations(perplexityResult),
          confidenceScore: perplexityResult.confidence,
          dataSourceQuality: this.calculateSourceQuality(perplexityResult.sources),
          enhancedVisualizationSuggestions: perplexityResult.visualizationSuggestions
        };
        
        // Notify about Perplexity-enhanced analysis
        NotificationCenter.showNotification({
          title: 'Enhanced Analysis Available',
          body: `Supply chain analysis enhanced with Perplexity AI (${perplexityDepth} depth)`,
          priority: 'medium',
          category: 'analysis',
          actionable: true,
          actionUrl: `/supply-chain/entity/${entityId}/enhanced`,
          dataPoints: {
            'Confidence': `${Math.round(perplexityResult.confidence * 100)}%`,
            'Enhanced Insights': `${perplexityResult.insights.length} items found`,
            'Analysis Level': perplexityDepth
          }
        }, 'supply-chain', 'enhanced-analysis');
      } catch (error) {
        console.error('Error enhancing analysis with Perplexity:', error);
        // Continue without Perplexity enhancement if it fails
      }
    }
    ...
    const resilience: SupplyChainResilience = {
      ...
      ...(Object.keys(perplexityEnhancedData).length > 0 ? { perplexityEnhanced: perplexityEnhancedData } : {})
    };
  ...
  /**
   * Create analysis prompt for Perplexity NLP
   */
  private createPerplexityAnalysisPrompt(
    entityId: string,
    ...
    perplexityDepth: string
  ): string {
    // Construct a detailed prompt based on analysis depth
    let prompt = `Analyze supply chain resilience for entity ${entityId} focusing on tariff impacts and vulnerabilities.`;
    
    // Add vulnerability hotspot information
    if (vulnerabilityHotspots.length > 0) {
      prompt += ` Identified vulnerability hotspots include: ${vulnerabilityHotspots.map(h => h.nodeName).join(', ')}.`;
    }
    
    // Add recommendation information
    if (recommendedActions.length > 0) {
      prompt += ` Initial recommendations include: ${recommendedActions.map(r => r.action).join(', ')}.`;
    }
    
    // Add depth-specific instructions
    if (perplexityDepth === 'comprehensive') {
      prompt += ` Provide a comprehensive analysis including market trends, regulatory outlook, and advanced risk mitigation strategies. Include quantitative metrics where possible.`;
    } else if (perplexityDepth === 'standard') {
      prompt += ` Provide a standard analysis of vulnerabilities and practical recommendations.`;
    } else {
      prompt += ` Provide a basic summary of key risks and immediate actions.`;
    }
  ...
  /**
   * Extract enhanced recommendations from Perplexity result
   */
  private extractEnhancedRecommendations(perplexityResult: any): any[] {
    if (!perplexityResult || !perplexityResult.recommendations || !Array.isArray(perplexityResult.recommendations)) {
      return [];
    }
    
    return perplexityResult.recommendations.map((rec: any) => ({
      action: rec.text || rec.action || 'Recommendation not specified',
      priority: rec.priority || 'medium',
      expectedImpact: rec.impact || 5.0,
  ...
  /**
   * Calculate data source quality from Perplexity sources
   */
  private calculateSourceQuality(sources: any[] | undefined): number {
    if (!sources || !Array.isArray(sources) || sources.length === 0) {
      return 0.5; // Default middle quality if no sources
    }
    
    // Calculate quality based on source count and recency
    const sourceCount = Math.min(sources.length, 10) / 10; // 0.1 to 1.0 based on source count
    ...
    try {
      const years = sources
        .map(s => s.year || s.date)
        .filter(Boolean)
        .map(d => typeof d === 'string' ? parseInt(d.slice(0, 4), 10) : d);
      
      if (years.length > 0) {
        const avgYear = years.reduce((sum, y) => sum + y, 0) / years.length;
        recencyScore = Math.min(1, Math.max(0, (avgYear - (currentYear - 5)) / 5));
      }
    } catch (error) {
      console.error('Error calculating source recency:', error);
    }
    
    // Combine scores (60% recency, 40% quantity)
    return (recencyScore * 0.6) + (sourceCount * 0.4);
  }
  ...
  private async fetchSupplyChainNodes(entityId: string, options: ResilienceAnalysisOptions): Promise<SupplyChainNode[]> {
    ...
    try {
      nodes = await this.fetchOntologyBasedSupplyChain(entityId, options.depth || 2);
    ...
    const sampleNodes = this.generateSampleSupplyChainNodes(entityId, options.depth || 2);
    return sampleNodes;
  }
  ...
  private async runMonteCarloSimulation(nodes: SupplyChainNode[], options: ResilienceAnalysisOptions): Promise<MonteCarloSimulationResults> {
    console.log(`Running Monte Carlo simulation with ${options.monteCarloSimulations} iterations`);
    ...
    // Run simulations
    const iterations = options.monteCarloSimulations || this.DEFAULT_MONTE_CARLO_ITERATIONS;
  ...
  private calculateSensitivityForFactor(node: SupplyChainNode, factor: string, scores: number[]): number {
    ...
    const sensitivityFactor = partialDerivative / (stdDev > 0 ? stdDev : 1);
  ...
  private calculatePartialDerivative(node: SupplyChainNode, factor: string, scores: number[]): number {
    ...
    const baseScore = this.calculateVulnerabilityScore(node, {
      ...
      includeAlternativeSuppliers: true,
  ...
  private async generateResilienceRecommendations(
    ...
    if (options.monteCarloSimulations && options.monteCarloSimulations >= 1000) {
      recommendations.push({
        priority: 'low',
   ...
   */
  private generateSampleSupplyChainNodesForTesting(): SupplyChainNode[] {
    return [
      {
        id: 'supplier-1',
